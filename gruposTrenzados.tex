...
$$f(x,y)=f(x)f(y)$$
para todo $x,y\in G$. Un homomorfismo biyectivo es un \textit{isomorfismo}. Cuando $f$ es un isomorfismo, decimos que $G$ y $G'$ son \textit{isomorfos} y lo denotamos por $G\cong G'$.
\end{defi}
...
Sea G un grupo. Diremos que un subgrupo $H$ de $G$ es \textit{normal} en $G$ si cumple una de las siguientes afirmaciones:
\begin{enumerate}
...
Una trenza elemental es una trenza de $\mathcal{B}_n$ en la que solo hay un cruce de hebras. Denotaremos por $\sigma_i^+$ la trenza que cruza la hebra $i+1$-ésima sobre la $i$-ésima trenza y por $\sigma_i^-$ a la trenza que cruza la $i$-ésima trenza sobre la $i+1$-ésima con $i = 1,\ldots,n-1$.
\end{defi}
